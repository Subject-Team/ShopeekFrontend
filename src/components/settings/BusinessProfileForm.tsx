    ...
    <div data-guide="settings-business-profile" className={cardClass}>
      ...
      <div className="flex items-start gap-3 p-3.5 sm:p-4 rounded-2xl bg-indigo-50/80 dark:bg-indigo-950/40 border border-indigo-200/80 dark:border-indigo-900/60 mb-5 sm:mb-6 text-indigo-900 dark:text-indigo-200">
        ...
        <div className="flex-1 min-w-0 text-xs leading-relaxed space-y-1">
          ...
          <p className="text-slate-700 dark:text-indigo-200/90 text-[11px] sm:text-xs">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-3 pt-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-slate-100 dark:border-slate-800 pb-2.5">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 text-xs font-bold hover:bg-indigo-100 dark:hover:bg-indigo-900 transition-colors self-start sm:self-auto cursor-pointer"
            >
          ...
          {links.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {links.map((link, idx) => (
                <div
                  key={idx}
                  className="flex flex-col sm:flex-row sm:items-center gap-2.5 p-2.5 sm:p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800"
                >
                  {/* Top row on mobile (type selector + icon + remove button) / inline on desktop */}
                  <div className="flex items-center justify-between sm:justify-start gap-2 w-full sm:w-auto">
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="px-2.5 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-xs border border-slate-200 dark:border-slate-700 font-medium focus:outline-hidden focus:ring-1 focus:ring-indigo-500 shrink-0 cursor-pointer"
                      >
                    ...
                    <button
                      ...
                      title="حذف لینک"
                      aria-label="حذف لینک"
                      className="sm:hidden p-1.5 rounded-xl text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/50 transition-colors shrink-0 disabled:opacity-40 cursor-pointer"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>

                  {/* URL or Handle Input - full width on mobile, flex-1 on sm+ */}
                  <div className="w-full sm:flex-1">
                    <input
                      ...
                      className="w-full px-3 py-2 sm:py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 text-xs border border-slate-200 dark:border-slate-700 focus:outline-hidden focus:ring-1 focus:ring-indigo-500"
                    />
                  </div>

                  {/* Remove Button on sm+ (desktop/tablet) */}
                  <button
                    type="button"
                    disabled={readOnly}
                    onClick={() => handleRemoveLink(idx)}
                    title="حذف لینک"
                    aria-label="حذف لینک"
                    className="hidden sm:inline-flex p-1.5 rounded-xl text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/50 transition-colors shrink-0 disabled:opacity-40 cursor-pointer"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col-reverse sm:flex-row sm:items-center justify-between gap-3.5">
          ...
          <button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-lg shadow-indigo-600/20 transition-all disabled:opacity-50 cursor-pointer"
          >
            {submitting ? (
              <>
