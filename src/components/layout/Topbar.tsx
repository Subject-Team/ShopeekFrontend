...
export const Topbar: React.FC<TopbarProps> = ({ onMenuClick, billing }) => {
  const {
    startDate,
    endDate,
    isHistorical,
  ...
  const planKey = billing?.plan?.key ?? user?.plan_key ?? 'trial';
  const planDisplay = billing?.plan?.name_fa || getPlanDisplay(planKey);
    ...
    <header className="h-16 sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-3 sm:px-4 lg:px-8 flex items-center justify-between transition-colors duration-200 dir-rtl">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <Link
          ...
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border border-sky-200/80 dark:border-sky-900/60 bg-sky-50/70 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300 hover:bg-sky-100/80 dark:hover:bg-sky-900/60 transition-all text-xs font-bold shadow-2xs group shrink-0"
          title="اعتبار باقی‌مانده"
        >
          <CreditIcon size={14} className="text-sky-600 dark:text-sky-400 shrink-0" />
          <span>{toGroupedPersianDigits(remainingCredits)}</span>
          <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-lg bg-sky-600 dark:bg-sky-500 text-white text-[11px] font-bold group-hover:bg-sky-700 dark:group-hover:bg-sky-400 transition-colors shadow-2xs">
            خرید اعتبار
          </span>
        </Link>
