...
import { X, Phone, Mail, MessageSquare, Send, Calendar, Clock } from 'lucide-react';
...
export const CustomerModal: React.FC<CustomerModalProps> = ({ customer, onClose, onRefresh }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] mx-2 sm:mx-4">
        {/* Modal Header */}
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-brand-500 text-white font-extrabold text-lg flex items-center justify-center shadow-lg shadow-brand-500/20">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 rounded-2xl p-4 border border-slate-200 dark:border-slate-700/60 space-y-3">
            ...
            <form onSubmit={handleAddNote} className="flex gap-2">
              <input
                ...
                className="flex-1 px-3.5 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs focus:outline-hidden focus:border-brand-500"
              />
              <button
                type="submit"
                disabled={submitting || !noteContent.trim()}
                className="px-4 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold transition-colors disabled:opacity-50 flex items-center gap-1.5 shrink-0"
              >
            ...
            {customer.interactions && customer.interactions.length > 0 ? (
              <div className="space-y-2.5">
                {customer.interactions.map(item => (
                  ...
                  >
                    <div className="p-2 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 mt-0.5">
