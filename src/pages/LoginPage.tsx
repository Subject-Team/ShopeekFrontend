...
export const LoginPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-900 text-slate-100 flex flex-col font-vazir dir-rtl selection:bg-brand-500 selection:text-white relative overflow-x-hidden">
      <SEO
      ...
      <header className="w-full max-w-md mx-auto flex items-center justify-between pt-6 px-4 z-20 shrink-0">
        <Link
          to="/"
          className="inline-flex items-center gap-2 text-xs font-semibold text-slate-300 hover:text-white bg-slate-800/80 border border-slate-700/60 px-3.5 py-2 rounded-xl backdrop-blur-md transition-all"
        >
          <Home className="w-4 h-4 text-brand-400" />
          <span>بازگشت به صفحه اصلی</span>
        </Link>

        <span className="text-xs text-slate-400 font-medium">سامانه تحلیلی شاپیک</span>
      </header>
      ...
      {/* Main Login Card Container with explicit vertical margins for footer spacing */}
      <main className="flex-1 flex items-center justify-center p-4 my-8 z-10">
        <div className="w-full max-w-md bg-slate-800/80 backdrop-blur-xl border border-slate-700/60 rounded-3xl p-6 md:p-8 shadow-2xl space-y-6">
          {/* Brand Header */}
          ...
          {errorMessage && (
            <div className="p-4 rounded-2xl bg-rose-500/15 border border-rose-500/30 text-rose-200 text-xs space-y-2.5 leading-relaxed">
              <div className="flex items-start gap-2.5">
                <AlertCircle className="w-4 h-4 text-rose-400 shrink-0 mt-0.5" />
                <div className="flex-1 font-medium">{errorMessage}</div>
              </div>
              <div className="pt-2 border-t border-rose-500/20 flex items-center justify-between">
                <span className="text-[11px] text-slate-300">نیاز به خرید یا تمدید اشتراک دارید؟</span>
                <Link
                  to="/contact"
                  className="inline-flex items-center gap-1 font-bold text-brand-300 hover:text-brand-200 underline underline-offset-4"
                >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode === 'register' && (
              <div className="pt-1">
                <label className="flex items-start gap-2.5 cursor-pointer text-xs text-slate-300 select-none leading-relaxed">
                  <input
                    type="checkbox"
                  ...
                  <div className="mt-0.5 text-brand-400 shrink-0">
                    {acceptedPrivacy ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4 text-slate-500" />}
                  </div>
                  <span>
                    من{' '}
                    <Link
