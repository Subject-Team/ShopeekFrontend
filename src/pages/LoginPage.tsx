...
import { normalizePersianDigits, formatPersianNumber } from '../utils';
...
export const LoginPage: React.FC = () => {
  ...
  const cleanPhone = useMemo(() => {
    return normalizePersianDigits(phone.trim().replace(/[^\d+]/g, ''));
  }, [phone]);
  ...
  const handleOtpLoginSubmit = async (e: React.FormEvent) => {
    ...
    const cleanOtp = normalizePersianDigits(otpCode.trim());
    if (!cleanOtp) {
          ...
          {step !== 'PHONE' && (
            <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-100/90 border border-slate-200 text-xs">
              <div className="flex items-center gap-2 text-slate-700 font-semibold">
                ...
                <span className="font-bold text-slate-900 dir-ltr">{formatPersianNumber(cleanPhone)}</span>
              </div>
              <button
                type="button"
